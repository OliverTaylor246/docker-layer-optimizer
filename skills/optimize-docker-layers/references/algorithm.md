...
`dlo build` prefers `docker buildx build --progress=rawjson`. It uses each structured vertex's explicit `cached`, `started`, `completed`, and `error` fields, and treats `FROM` completion as image resolution rather than rebuilt filesystem work. If Buildx rejects `rawjson`, auto mode retries with the less robust plain-progress parser. Internal BuildKit vertices are excluded from Dockerfile-step counts.
...
Failed builds advance the context-change snapshot so repeated attempts do not count the same edit repeatedly, but they never replace the last successfully inspected image-layer or registry baseline. A successful `--push` build resolves the pushed OCI/Docker index and platform manifest through Buildx, then compares compressed filesystem-layer blob digests and declared sizes with the previous successful push for the same target. Unmatched compressed bytes are not actual uploaded bytes because registry-side blob presence and transport behavior are not observed. Dockerfile step counts and image layers are intentionally separate: a build step need not create exactly one filesystem layer.
...
State is keyed by a hash of the canonical project path and stored under the user cache (`~/Library/Caches`, `XDG_CACHE_HOME`, or `LOCALAPPDATA`). `DLO_CACHE_DIR` overrides the base. Events are JSON Lines; context and successful image baselines are stored atomically in `snapshot.json`. A global lock protects state updates and a per-target lock serializes measured builds that would otherwise race on the same baseline.
